...
npm run e2e            # Playwright：119 个端到端用例 / 27 个文件（导入保真与导入摘要 / 多格式兼容 /
                       ...
                       #   白屏回归（冷标签重建/会话恢复中导入、绑定错位自愈）与加载失败兜底（分块 404、入口没起来）/
...
| `npm run e2e` | 端到端 **119 例全量**（两阶段串起来） | **~3 分钟** |
| `npm run e2e:fast` | 只跑"主项目"105 例（跳过 14 例串行用例） | ~2 分钟 |
...
12. **"页面空白 / 打不开，刷新又好了"有三种不同成因，别只盯着一种**（用户实测反馈，见 P3「修复 X」）：
