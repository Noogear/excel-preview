...
> 右键菜单（**复制到工作区 / 剪切到工作区** / 与工作区互换 / 复制内容（**单块含格式，可直接粘到 Excel/WPS**）/ 粘贴 / 清空内容（**保留格式且可撤销**）/ 撤销重做）、
> **复制出去**：`Ctrl+C` 或右键「复制内容」→ 系统剪贴板同时写 **TSV 纯文本 + 带格式 HTML**
> （字体/字色/底色/边框/对齐/合并/列宽/行高），粘到 Excel、WPS、腾讯文档、Google Sheets 都能分格；
> 非安全上下文（`http://局域网IP`）自动退回 `execCommand` 复制，不会静默失败；
> **粘进来**：外部带格式内容只会写值，目标格格式一格不变（也不会引入单元格内富文本）、
...
>
> **最快的加入方式（快速加入）**：在**选择模式**或**点击互换模式**下点一下表格里的单元格（Ctrl+点选可多块），
> **3 秒内**再点工作区面板的**空白处**，选区就整片收进来了（走的是同一个入口：逐格拆分、跳过空内容、
> 按 id 去重、**可撤销**）。规则很克制，避免误加：拖到面板里松手不算（那是"拖入工作区"）；
> 点搜索框/按钮/条目不算；同一次选区连点两下也只加一次；超过 3 秒后点空白只是点空白；
> **拖拽模式刻意不参与**（那一模式的手势语言就是"按住拖"，少一个隐式动作更不容易误触）。
>
> **导出面板只显示能用的项**：菜单按"当前真的能不能用"决定放不放进来 —— 有打开的文件才有「.xlsx（保真）」，
> 有工作表就有「.csv」，「经本机 Excel」的 `.ods/.xls/.xlsb` 只在**本地版 + 本机装了 Excel** 时出现
> （静态托管版里它们**直接隐藏**，不再给一个点了没反应的置灰项）。
>
...
npm test               # Vitest：465 个用例 / 30 个文件（解析器（OOXML/CSV/ODS/XLS）/ 样本真值 /
                       #   导入适配与格式判定 / xlsx 合成写入器 / 只读门禁 / 交互与三种模式 /
                       #   像素↔格换算 / 多区域选区 / 选区与合并块对齐 / 工作区逐格拆分与空内容跳过 /
                       #   「+」面板的识别与预览文案、目标范围换算 /
                       #   日期取值口径 / 提醒框与滚动条几何 / 多标签常驻窗口 / 持久化 / 导出保真 /
                       #   右键菜单 / 历史 / 撤销快捷键归属 / 崩溃兜底卡 / 性能护栏）
npm run e2e            # Playwright：131 个端到端用例 / 30 个文件（导入保真与导入摘要 / 多格式兼容 /
                       #   只读约束 / 导出与导出菜单（含本地 Excel 桥导出 .ods）/ 三种交互模式与选区外观 /
                       #   **导出保真**（不改就导出=除 calcPr 外逐字节一致；改了值=特性元素与只读部件原样保留；Excel 能打开）/
                       #   剪贴板（Ctrl+C 与右键复制带格式出去、外部带格式内容粘进来不改格式）/
                       #   内存泄漏回归（插图 blob url 随关标签/冷存/拆卸归还）/
                       #   不连续多区域 / 工作区与跨表 / 工作区快速加入（选中后点空白）/ 工作区拖放守卫（不自我复制、过期选区不误加）/
                       #   工作区撤销与重做（含"拖回表格可撤销""面板剪切/复制两条语义""打开文件不留幽灵条目"）/
                       #   白屏回归（冷标签重建/会话恢复中导入、绑定错位自愈、关掉最后一个标签后再打开文件）/
                       #   加载失败兜底（分块 404 自动重来、入口没起来的看门狗）/
                       #   静态形态（子路径托管可用 + 启动预算 + 桥能力降级）/
                       #   右键菜单 / 历史 / 持久化 / 日期单元格 / 缩放组件 /
                       #   滚动与滚动条 / 坐标一致性 / 提示统一 / 选区与合并块 / 文件拖放与打开格式 /
                       #   多标签内存 / 热更新后标签不乱 / 百万格性能）
npm run bridge:clean   # 收拾"转换桥"可能残留的无窗口 Excel（COM 自动化实例）——
                       #   只杀命令行带 -Embedding 的，不动你自己开的 Excel；加 -DryRun 只看不杀
```
...
| `npm test` | 单元 467 例（Vitest 多线程跑文件） | ~15–30 s（看机器忙不忙） |
| `npm run e2e` | 端到端 **131 例全量**（三段串起来：main → sensitive → hot-reload） | **~3–4 分钟** |
| `npm run e2e:fast` | 只跑"主项目"112 例（跳过 17 例串行 + 2 例热更新） | ~2 分钟 |
| `npm run e2e:sensitive` | 只跑那 17 例串行用例 | ~1.5 分钟 |
| `npm run e2e:hot-reload` | 只跑 2 例热更新用例（**必须是独立进程**，见下） | ~20 s |

**为什么是三个阶段**（main 并发 + sensitive 串行 + hot-reload 独立进程）—— 并发能把 8 分钟压到 3 分钟，
但有几类用例天生不能和别人抢机器，放一起会**测出假失败**（不是放宽断言，而是让它们各自在干净环境里跑）：

| 用例 | 为什么必须单独跑 |
| --- | --- |
| `hot-reload.spec.ts` | 它会**改写 `src/App.tsx`** 触发真实热更新；实测它与 `perf.spec.ts` **同处一个 playwright 进程**时，用例全部通过但**进程永不退出**（各自单跑都秒退），所以它走**独立的第三次调用**，而不是塞进某个项目里 |
...
| `clipboard.spec.ts` | 读写的是**真实系统剪贴板**（机器级共享资源）：并发跑时别人的复制会盖掉它的哨兵值，测出来的失败不是真失败 |
...
- `hot-reload.spec.ts` ②：**自动保存落盘**不再用写死的 `waitForTimeout(2600)` ——
  自动保存是"每 2000ms 查一次指纹 + 变化了再去抖 800ms"，最坏 2800ms 才进库，
  写死 2600ms 正好卡在边界上（相位偏 200ms 就随机红，实测同一份代码连着红两次）。
  现在改成**轮询"库里真的有这份会话了"**（断言更严：真的在验证自动保存会写库，而不是绕过去）；
  末尾"等恢复跑完"也改成等 `session:restore-done` 日志，同样不再猜时间。
...
12. **"页面空白 / 打不开，刷新又好了"有四种不同成因，别只盯着一种**（用户实测反馈，见 P3「修复 X」）：
   ① 活动单元与画布绑定错位（并发建簿）→ 串行闸 + 自检自愈；② 渲染期抛异常（React 18 会卸载整棵树）→ 错误边界；
   ③ **分块/入口脚本没下下来**（静态托管发布换了 hash、浏览器还拿着旧 `index.html`，或弱网截断）→
   动态 import 直接 reject，以前会**永远停在骨架屏**；而按 F5 拿到新 HTML 就好 —— **和用户描述完全对得上**。
   这一条现在有确定的复现方式（e2e 里把分块请求 `route.abort()`），修法是"自动重来一次 + 可读卡片 + 看门狗"。
   ④ **关掉最后一个标签**（用户后来给的确切步骤：打开文件 → 等备份落盘 → 刷新 → 关掉还原出来的标签 → 再打开文件）→
   `disposeUnit` 释放掉唯一的单元，Univer 把"当前单元"置成 `null`、把 canvas 一起摘掉（实测 `canvasCount: 0`），
   之后再打开文件**模型全对但画布回不来**，连新建工作簿都没用（渲染根已被回收，只能刷新）。
   **修法：关最后一个标签时"先摆占位单元、再释放"**（`tab:placeholder-created`），单元数永远不为 0。
   > 这条教训要记牢：**Univer 里"一个单元都没有"不是空状态，而是坏状态** ——
   > 启动时之所以正常，是因为那时候摆着示例工作簿（`p0-workbook`）。
13. **别用写死的 `sleep` 等自动保存**：它是"每 2000ms 查一次指纹 + 变化了再去抖 800ms"，最坏 2800ms 才进库；
   写 `waitForTimeout(2600)` 就是卡在边界上，相位偏 200ms 就随机红（`hot-reload.spec.ts` ② 实测踩过）。
   要等落盘就**轮询"库里真的有了"**；要等恢复跑完就等 `session:restore-done` 日志。
