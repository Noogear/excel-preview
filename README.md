...
npm run e2e            # Playwright：120 个端到端用例 / 27 个文件（导入保真与导入摘要 / 多格式兼容 /
...
| `npm test` | 单元 465 例（Vitest 多线程跑文件） | ~15–30 s（看机器忙不忙） |
| `npm run e2e` | 端到端 **120 例全量**（两阶段串起来） | **~3 分钟** |
| `npm run e2e:fast` | 只跑"主项目"106 例（跳过 14 例串行用例） | ~2 分钟 |
