...
npm run e2e            # Playwright：105 个端到端用例 / 24 个文件（导入保真与导入摘要 / 多格式兼容 /
                       #   只读约束 / 导出与导出菜单（含本地 Excel 桥导出 .ods）/ 三种交互模式与选区外观 /
                       #   不连续多区域 / 工作区与跨表 / 工作区拖放守卫（不自我复制、点空白不误加入）/
...
| `npm run e2e` | 端到端 **105 例全量**（两阶段串起来） | **~3 分钟** |
| `npm run e2e:fast` | 只跑"主项目"91 例（跳过 14 例串行用例） | ~2 分钟 |
