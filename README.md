...
npm run e2e            # Playwright：128 个端到端用例 / 29 个文件（导入保真与导入摘要 / 多格式兼容 /
...
| `npm run e2e` | 端到端 **128 例全量**（两阶段串起来） | **~3 分钟** |
| `npm run e2e:fast` | 只跑"主项目"109 例（跳过 19 例串行用例） | ~2 分钟 |
| `npm run e2e:sensitive` | 只跑那 19 例串行用例 | ~2 分钟 |
