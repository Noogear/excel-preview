/**
 * 只读约束（合并自原 p0.spec.ts 与 p3-constraints.spec.ts 的约束①②）。
 *
 * 产品的核心约束：**只允许改单元格文字内容**，字体/边框/背景/结构/对象一律改不动。
 *
 * 保留的 3 条：
 *   ① 22 条破坏性命令（样式/结构/对象/子表）逐条被拦下，且拦截前后**文档指纹零变化**
 *      （原 p0 的"单条 set-style 被拦"是它的子集，已删）
 *   ② mutation 层兜底：夹带样式的写入必须把样式剥离干净、但值要写进去（防"粘贴带格式"绕过闸门）
 *   ③ 真实键盘输入：内容写得进去、样式一字不变、且可撤销（防闸门误伤编辑路径）
 *
 * 已删除：p0 的"swapValues 确定性互换"（与 workspace.spec.ts 的互换用例重复）、
 * p0 的 p0-findings.json 事实记录（探索性，不是断言）。
 */
