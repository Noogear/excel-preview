...
/** 目录里最新的修改时间（用来判断产物是不是"旧代码"） */
function newestMtime(dir: string): number {
  let newest = 0;
  for (const name of readdirSync(dir)) {
    const full = join(dir, name);
    const info = statSync(full);
    if (info.isDirectory()) newest = Math.max(newest, newestMtime(full));
    else newest = Math.max(newest, info.mtimeMs);
  }
  return newest;
}

/**
 * 产物必须是**当前代码**构建出来的，否则这组用例会拿旧产物测新行为（实测踩过：
 * 导出菜单改成"不可用就隐藏"之后，`dist/` 还是上一版，于是断言 `toHaveCount(0)` 失败）。
 * 发现 `src/**` 比 `dist/index.html` 新就地重建一次（~13 秒，只在过期时发生）。
 */
function rebuildStaticIfStale(): void {
  if (!HAS_DIST) return;
  try {
    const distTime = statSync(join(DIST, 'index.html')).mtimeMs;
    const srcTime = Math.max(newestMtime(resolve('src')), statSync(resolve('index.html')).mtimeMs);
    if (srcTime <= distTime) return;
    console.log('[static-form] 产物比源码旧 → 重新构建静态版 …');
    spawnSync(process.execPath, [resolve('node_modules/vite/bin/vite.js'), 'build', '--mode', 'static'], {
      cwd: process.cwd(),
      stdio: 'inherit',
    });
  } catch (error) {
    console.log(`[static-form] 产物新旧检查失败（继续用现有产物）：${String(error)}`);
  }
}
...
test.describe('静态形态：能托管、启动快、桥能力被降级', () => {
   ...
   * 导出菜单里**不该出现**那三项（.ods/.xls/.xlsb —— 都要经本机 Excel）。
   *
   * 用户要求（"导出按钮的面板进行优化，无法使用的功能直接隐藏"）：
   * 以前是"置灰 + 写一行说明"，现在直接**不出现在菜单里**，只留下真能用的项。
   */
  test('③ 静态形态：导出菜单只留能用的项（桥相关项直接隐藏）', async ({ page }) => {
    ...
    for (const id of ['export-ods', 'export-xls', 'export-xlsb']) {
      await expect(page.locator(`[data-testid="context-menu-${id}"]`), `${id} 不该出现在静态版的菜单里`).toHaveCount(0);
    }
    // 菜单里也不该留"看着能用却点不动"的项
    const disabledCount = await page
      .locator('[data-testid^="context-menu-export-"]')
      .evaluateAll((nodes) => nodes.filter((node) => (node as HTMLButtonElement).disabled).length);
    expect(disabledCount, '菜单里不该有置灰的导出项').toBe(0);
  });
});
