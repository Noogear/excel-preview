/**
 * 右键菜单（原 p3-context-menu.spec.ts 4 条 → 3 条）。
 *
 ...
 *
 * 新数据模型（P4）：工作区条目 = 1×1 单格，任何入口都按**行优先**把区域拆开、跳过空单元格。
 * 所以这里不再断言"1 个条目 / rows < 25"，改断言"条目数 = 该区域非空格子数"（现算，不写死）。
 *
 * 已删除："Esc 关闭菜单且不残留节点"（菜单关闭已由 ① 的 `context-menu` 计数断言覆盖）。
 *
